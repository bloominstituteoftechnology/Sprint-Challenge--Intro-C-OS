...
1. The operating system determines which programs should run in what order and how much time should be allowed for each programs before giving another program a turn through a process scheduler. (Note - Programs are applications.) It also manages sharing internal memory between the multiple applications.
